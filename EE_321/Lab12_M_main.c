/*
* EE321 Lab 12 dc Motor State Machine
*
* Casey Mauldin
* Norwich University
* 12JUN22
...
unsigned int    countArray[] = { 0, 0, 0, 0 };      // counts between captures
unsigned int    encoderCount = 0;                   // sum 4 count values
unsigned int    encoderSum = 0;                     // sum 4 count values
unsigned char   countIndex = 0;                     // index to track encoderCount values
unsigned int    rpm = 0;
// main.c
void main(void) {
    ...
    P2DIR |= PMW_OUT + TAC_OUT; // Bit 6 of P2DIR sets P2.6 as output & will be used for PWM
    P2SEL |= PMW_OUT + TAC_OUT; // P2.6 TA0.1 option select
    P2OUT &= ~BLOCK;

    // Set up for Tachometer

    TA1CCTL2 = CAP + CM_2 + CCIS_1 + CCIE;
    ...
    P1SEL |= BIT4;
    //__enable_interrupt();
    rpm = 144 / encoderCount;      // magnitude RPM
        if(P2IN & BIT2)             // check if CCW LED is on
            rpm = -rpm;         // negative if CCW
    while(1)
    {
        ADC10CTL0 &= ~ENC;
...
/*************************************************************************/
// Timer_A3 Interrupt Vector (TA1IV) handler
#pragma vector=TIMER1_A1_VECTOR
__interrupt void Timer_A12(void)
{
 __no_operation();
 switch( TA1IV )
 {
 ...
 case 4: // CCR2
 {
     ...
     if(TA1CCR2>TA1CCR2)                 // ignore negative values
     {
    encoderSum -= countArray[countIndex];       // subtract the oldest value
    countArray[countIndex] = TA1CCR2 - TA1CCR2; // current - last count
    encoderSum += countArray[countIndex];       // add the newest value
    encoderCount = encoderSum >> 2;             // divide sum by 4
    countIndex = (countIndex + 1) & 0x03;       // increment the index and reset if >3
     }
 }
 break;
